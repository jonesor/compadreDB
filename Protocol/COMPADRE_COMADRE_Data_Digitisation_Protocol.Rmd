...
__FRANK__ - Frankenstein matrix - The author used demographic data from at least two other papers and calculated a *new* matrix population model out of those data. In those cases, we typically have different study sites and years of study. For these data we do not calculate averages or ranges but instead enter __FRANK__.

__Simulation__ - A simulated matrix - The matrix model is a simulation, and perhaps only loosely based on a specific real species. Typically these simulated matrices are used to explore the behaviour of a population in a theoretical way. In those cases, there is no geographic location, ecoregion etc. For these data we do not calculate averages or ranges but instead enter __Simulation__.
...
* SpeciesAccepted - Currently accepted Latin name. This information is is obtained from The Plant List (http://www.theplantlist.org) for plants and the Catalogue of Life (http://www.catalogueoflife.org/col/) for animals.
