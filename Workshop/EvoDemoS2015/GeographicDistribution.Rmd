...
First, subset mean matrices for all Carnivora in the wild in the
 Northern hemisphere, with no issues for survival >1, for
 which matrices have been split into **A** = **U** + **F** + **C**, and for  which reproduction was explicitly modeled.
...
s1 <- seq(min(x$metadata$lambdas,na.rm=TRUE),max( x$metadata$lambdas,na.rm=TRUE),length.out = 100)
```
...
points(jitter(x$metadata$LON,amount=0.6),jitter(x$metadata$LAT,amount = 0.6),col = colVect[findInterval(x$metadata$lambdas,s1)],cex=2,pch=16)
```